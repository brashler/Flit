...
const flatPositions = () => Array.from(world.positions.subarray(0, world.count * 3));
const flatRadii = () => {
    const out = [];
    for (let i = 0; i < world.count; i += 1)
        out.push(world.radiusOf(i));
    return out;
};
...
server.registerTool('flit_info', {
    title: 'Engine info',
    description: 'Flit engine metadata: what it is, current world state, and the repo. Good first call.',
    inputSchema: {},
}, async () => asJson({
    name: 'flit',
    about: 'Tiny sphere-physics engine for three.js: gravity, impulse contacts, ' +
...
server.registerTool('flit_reset', {
    title: 'Reset world',
...
server.registerTool('flit_add_particles', {
    title: 'Add particles',
...
server.registerTool('flit_spawn', {
    title: 'Spawn preset scene',
    description: 'One-shot scene spawners for games/demos: "rain" drops balls from above, ' +
...
server.registerTool('flit_step', {
    title: 'Step simulation',
    description: 'Advance the world. Returns flat xyz positions shaped for direct use in ' +
...
server.registerTool('flit_state', {
    title: 'Read state',
...
server.registerTool('flit_raycast', {
    title: 'Raycast',
    description: 'Cast a ray through the scene (Amanatides-Woo voxel walk over the broadphase grid). ' +
...
}, async ({ origin, direction, maxDistance }) => asJson({ hits: world.raycast(origin, direction, maxDistance ?? Infinity) }));
